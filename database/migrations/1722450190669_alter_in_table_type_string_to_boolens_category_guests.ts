...
export default class AlterIsActivedAndIsDeletedTypesInguests extends BaseSchema {
  protected tableName = 'category_guests'
  ...
  public async up() {
    ...
    await this.schema.alterTable(this.tableName, (table) => {
      table.boolean('is_actived').notNullable().defaultTo(true).after('description')
      table.boolean('is_deleted').notNullable().defaultTo(false).after('description')
    })
  }
